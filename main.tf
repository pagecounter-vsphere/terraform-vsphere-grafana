...
resource "vsphere_virtual_machine" "grafana-vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "curl -sLo /tmp/grafana-server.sh https://raw.githubusercontent.com/kikitux/curl-bash/master/provision/grafana-server.sh",
      "sudo -E bash /tmp/grafana-server.sh",
...
output "guest_ip_address" {
  value = "${vsphere_virtual_machine.grafana-vm.guest_ip_addresses[0]}"
}
